...
def Simulate(tag, genome, readstype, cores, haplotype, chromosome, start, end, label, model_qc, accuracy, coverage, allelic, length, ratio, output, haplonum, clone,renamer):
	...
	if readstype == 'ONT':
		...
		with open(os.path.abspath(output + '/region.tmp.sam'), 'w') as samout:
			...
			subprocess.call(['minimap2', '-ax', 'map-ont', '--MD', '-Y', '-t', str(cores), new_mmi, os.path.abspath(output + '/sim_0001.fastq')], stdout=samout, stderr=open(os.devnull, 'wb'))
		...
		with open(os.path.abspath(output + '/region.tmp.sam'), 'w') as samout:
			...
			subprocess.call(['minimap2', '-ax', 'map-pb', '--MD', '-Y', '-t', str(cores), new_mmi, os.path.abspath(output + '/sim_0001.fastq')], stdout=samout, stderr=open(os.devnull, 'wb'))
